...
function readJournalConfig(target) {
  const buffer = readOptional(resolveWithin(target, ".journal/config.json"));
  if (!buffer) return {};
  return parseJson(buffer, ".journal/config.json");
}
...
function share(options) {
  ...
  const ignored = gitIgnored(target, `.journal/work/${work.slug}`, options);
  ...
  if (work.visibility === "team_shared") {
    return { action: "share", target, work: work.slug, visibility: "team_shared", changed: false, gitIgnored: ignored, warning };
  }
  ...
  return { action: "share", target, work: work.slug, previousVisibility: work.visibility, visibility: "team_shared", changed: true, dryRun: !!options.dryRun, gitIgnored: ignored, warning };
}
...
function sync(options) {
  const target = checkTarget(options.target || process.cwd());
  const config = syncConfig(target);
  if (config.enabled !== true) {
    return { action: "sync", target, skipped: true, reason: "sync is not enabled" };
  }
  if (config.mode === "colocated") {
    return { action: "sync", target, skipped: true, reason: "sync mode is colocated" };
  }
